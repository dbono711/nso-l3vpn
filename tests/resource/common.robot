...
Setup NSO Session
    ${SESSION_EXISTS}=    Session Exists       NSO
    Run Keyword Unless    ${SESSION_EXISTS}    Create Session   NSO   ${NSO_URL}    auth=@{NSO_AUTH}    verify=False    disable_warnings=True

Delete All Instances of ${PACKAGE}
    Delete Request    NSO    ${NSO_BASE_PATH}?fields=${PACKAGE}:${PACKAGE}
...
Test CREATE Dry-Run
    ...
    ${RESP}=    Post Request    NSO    ${NSO_BASE_PATH}?dry-run=xml   data=${CREATE_PAYLOAD}    headers=${HEADERS}
...
Test CREATE
    ${RESP}=    Post Request    NSO    ${NSO_BASE_PATH}   data=${CREATE_PAYLOAD}    headers=${HEADERS}
    Should Be Equal As Integers    ${RESP.status_code}    201   ${RESP.text}

Test READ
    ${CRUD_TEST}=   Set Variable   Test READ
    ${RESP}=    Get Request    NSO    ${NSO_BASE_PATH}${CREATE_SVC_INST.package}:${CREATE_SVC_INST.package}=${KEYSTRING}
...
Test UPDATE Dry-Run
    ...
    ${RESP}=    Patch Request    NSO   ${NSO_BASE_PATH}${CREATE_SVC_INST.package}:${CREATE_SVC_INST.package}=${KEYSTRING}?dry-run=xml   data=${UPDATE_PAYLOAD}    headers=${HEADERS}
...
Test UPDATE
    ${RESP}=    Patch Request    NSO   ${NSO_BASE_PATH}${CREATE_SVC_INST.package}:${CREATE_SVC_INST.package}=${KEYSTRING}   data=${UPDATE_PAYLOAD}    headers=${HEADERS}
    Should Be Equal As Integers    ${RESP.status_code}    204   ${RESP.text}

Test DELETE Dry-Run
    ${CRUD_TEST}=   Set Variable   Test DELETE Dry-Run
    ${RESP}=    Delete Request    NSO    ${NSO_BASE_PATH}${CREATE_SVC_INST.package}:${CREATE_SVC_INST.package}=${KEYSTRING}?dry-run=xml
...
Test DELETE
    ${RESP}=    Delete Request    NSO    ${NSO_BASE_PATH}${CREATE_SVC_INST.package}:${CREATE_SVC_INST.package}=${KEYSTRING}
    Should Be Equal As Integers    ${RESP.status_code}    204   ${RESP.text}
