...
class L3vpn(Network):
    ...
    def __setup_vrf(self, device, base_vars=[]):
        ...
        # Derive Route Distinguisher (RD) from PE Loopback0 IP and Service ID
        rd = f'{self.get_loopback_ip(device.device_name, 0)}:{self.service.vpn_id}'
        vrf_vars.add('RD', rd)
    ...
    def __setup_intf(self, device, intf, base_vars=[]):
        ...
        # Ensure the interfaces selected are allowed
        self.check_allowed_interface(
            device, intf_type, intf_id, intf.port_mode)
    ...
    def __setup_static_routing(self, device, static, base_vars=[]):
        ...
        for ipv4_destination_prefix in static.ipv4_destination_prefix:
            static_vars.add(
                "IPV4-DEST-PREFIX", ipv4_destination_prefix.ipv4_prefix)
            ...
            for intf in device.interface:
                for forwarding in ipv4_destination_prefix.ipv4_forwarding.as_list():
                    intf_network = ipaddress.ip_interface(
                        intf.ipv4_local_prefix).network
                    if not self.check_host_in_network(forwarding, intf_network):
                        raise Exception(
                            f'Forwarding hop "{forwarding}" for static route "{ipv4_destination_prefix.ipv4_prefix}" does not fall within the "{intf_network}" prefix on "{intf.name}"')
        ...
        for ipv6_destination_prefix in static.ipv6_destination_prefix:
            static_vars.add(
                "IPV6-DEST-PREFIX", ipv6_destination_prefix.ipv6_prefix)

            for intf in device.interface:
                intf_network = ipaddress.ip_interface(
                    intf.ipv6_local_prefix).network
                if not self.check_host_in_network(ipv6_destination_prefix.ipv6_forwarding, intf_network):
                    raise Exception(
                        f'Forwarding hop "{ipv6_destination_prefix.ipv6_forwarding}" for static route "{ipv6_destination_prefix.ipv6_prefix}" does not fall within the "{intf_network}" prefix on "{intf.name}"')
            static_vars.add(
                "IPV6-FORWARDING", ipv6_destination_prefix.ipv6_forwarding)
            template_static.apply('l3vpn-static', static_vars)
    ...
    def configure(self):
        ...
        for device in self.service.device:
            ...
            if device.ce_routing.static:
                self.__setup_static_routing(
                    device, device.ce_routing.static, base_vars)
