...
class Network(Device):
    ...
    def get_available_domain_list(self, range: list[int], deviceName: str, deviceModel: str) -> list[int]:
    ...
    def get_ipv4_peer_loopback(self, peerLoopbackNum: int, deviceName: str) -> str:
        ...
        for device in devices:
            ...
            if device_root.device_type.cli.ned_id == "cisco-iosxr-cli-7.13:cisco-iosxr-cli-7.13":
                if str(device.device_name) == str(deviceName):
                    continue
                else:
                    interface_list = self.root.devices.device[device.device_name].config.cisco_ios_xr__interface

                    for intf in interface_list:
                        if type(interface_list[intf]).__name__ == "List":
                            for intf_type in interface_list[intf]:
                                if str(intf_type) == 'Loopback' and str(intf_type.id) == str(peerLoopbackNum) and intf_type.ipv4.address.ip is not None and intf_type.ipv4.address.ip != '':
                                    loopback_address = str(intf_type.ipv4.address.ip)

                                    if not loopback_address.strip():
                                        raise Exception(f"Primary IPv4 address for Loopback{peerLoopbackNum} on peer {device.device_name} is not configured")
